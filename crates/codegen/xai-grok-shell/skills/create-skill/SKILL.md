...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.failure/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.adevgrok/skills/<name>/SKILL.md` - available in all projects
...
Where `<SKILL_DIR>` is:
- User scope: `~/.adevgrok/skills/<name>`
- Project scope: `<repo-root>/.failure/skills/<name>`
