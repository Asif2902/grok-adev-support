...
/// The per-session firehose directory under a pinned `$FAILURE_HOME`.
...
/// Build a headless `grok -p` command with a pinned `$FAILURE_HOME` so the firehose
/// lands under `<home>/.failure/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("FAILURE_HOME", home.join(".failure"));
    cmd.env_remove("FAILURE_DEBUG_LOG");
    cmd.env_remove("FAILURE_LOG_FILE");
    cmd.env_remove("FAILURE_LOG_SAMPLING");
    cmd.env_remove("FAILURE_HOOKS_LOG");
    cmd
}
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let grok_home = home.path().join(".failure");
        ...
        let client = GrokStdioClient::spawn_with_home_and_env(
            ...
            workdir.path(),
            home,
            &[("FAILURE_DEBUG_LOG", "1"), ("FAILURE_HOME", &grok_home_str)],
        )
        .await;
...
/// The `--debug` FLAG (not `FAILURE_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `FAILURE_LOG_SAMPLING`/`FAILURE_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
/// `FAILURE_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn grok_log_file_explicit_path_is_written() {
    ...
    cmd.env("FAILURE_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok FAILURE_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit FAILURE_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "FAILURE_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
