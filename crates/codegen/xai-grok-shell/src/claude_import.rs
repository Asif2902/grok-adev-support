...
/// overwritten).
///
/// Project items are written to `<repo_root>/.failure/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.failure/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".failure").join("config.toml");
        ...
        let hooks_dir = project_root.join(".failure").join("hooks");
...
/// (Claude-compatible). The native hooks loader scans `.failure/hooks/*.json`
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_FAILURE_CLAUDE_MARKER_OVERRIDE") };
        }
    }
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `FAILURE_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_FAILURE_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
