...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.adevgrok/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
/// (e.g. `~/.adevgrok/...`) — prefer the managed `~/.adevgrok/bin/grok` symlink. After an
...
/// calling binary.
///
/// Falls back to `~/.adevgrok/bin/grok` only when `current_exe()` is unavailable.
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    for key in [
        "FAILURE_DEBUG_LOG",
        "FAILURE_HOOKS_LOG",
        "FAILURE_LOG_SAMPLING",
        "FAILURE_INSTRUMENTATION",
    ] {
    ...
    let leader_log = std::env::var("FAILURE_LEADER_LOG")
