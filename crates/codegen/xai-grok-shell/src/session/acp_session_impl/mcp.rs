...
impl SessionActor {
    ...
    /// or removed it from `~/.adevgrok/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.adevgrok/config.toml` + the project
    /// `.failure/config.toml`) on every call. The auto-restart task
    /// calls this at most:
