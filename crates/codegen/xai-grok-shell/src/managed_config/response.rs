...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that FAILURE_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.adevgrok. Make sure the directory exists and is writable.\n  ({0})"
    )]
