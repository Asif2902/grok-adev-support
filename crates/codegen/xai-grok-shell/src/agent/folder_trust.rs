...
//! inside a cloned repository (`.mcp.json`, project `.failure/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.failure/lsp.json`). Those configs
...
/// folder is gated (so an `.failure/lsp.json`-only repo still has a non-empty reason
...
///
/// Sources: project `.failure/config.toml [mcp_servers]` (NOT the user-tier global
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.failure/config.toml [mcp_servers]` entries tagged project (the loader's key
...
mod tests {
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; FAILURE_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because FAILURE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. FAILURE_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because FAILURE_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. FAILURE_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because FAILURE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; FAILURE_HOME-isolated store; FAILURE_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let grok_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", grok_home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        ...
        std::fs::create_dir_all(home.path().join(".failure").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. FAILURE_HOME-isolated so the trust store is empty;
        // FAILURE_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. FAILURE_HOME-isolated so the trust store is empty;
        // FAILURE_FOLDER_TRUST unset so the default-on feature flag applies.
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). FAILURE_HOME-isolated so the trust store is empty.
        use xai_grok_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. FAILURE_HOME-isolated (empty store).
        use xai_grok_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".failure").join("agents");
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.failure/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). FAILURE_HOME-isolated (empty
        // store); FAILURE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".failure").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + FAILURE_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. FAILURE_HOME-isolated so the seeded store is the temp one;
        // FAILURE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // FAILURE_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(xai_grok_version::TEST_VERSION_ENV);
        if option_env!("FAILURE_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.failure/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. FAILURE_HOME-isolated (empty store);
        // FAILURE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".failure").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // FAILURE_HOME-isolated (empty store); FAILURE_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".failure").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // FAILURE_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); FAILURE_FOLDER_TRUST unset so
        ...
        let plugin = tmp.path().join(".failure").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // FAILURE_HOME-isolated so the grant writes to a temp store; FAILURE_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".failure").join("hooks");
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.failure/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.failure/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.failure/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".failure");
        std::fs::create_dir_all(&grok).unwrap();
        std::fs::write(grok.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.failure/config.toml [mcp_servers]`).
    ...
    /// `.failure/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("FAILURE_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no FAILURE_VERSION) auto-trusts, so the gate would never engage without this.
        ...
        unsafe { std::env::remove_var("FAILURE_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // FAILURE_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("FAILURE_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".failure").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".failure").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with FAILURE_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. FAILURE_HOME-isolated so the
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. FAILURE_HOME-isolated so the store starts
        // empty; `#[serial]` because FAILURE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // FAILURE_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; FAILURE_HOME-isolated and
        // FAILURE_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("FAILURE_HOME", home.path());
        let _flag = EnvGuard::unset("FAILURE_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".failure").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".failure").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.failure/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".failure")).unwrap();
        std::fs::write(tmp.path().join(".failure").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
