...
/// `<cwd>/.failure/` (catches `<cwd>/.failure/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.failure/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.failure/config.toml` so the loop above
        // watches `<cwd>/.failure/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.failure/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.failure/config.toml` trigger a
    ...
    /// `<cwd>/.failure/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.failure/` does not yet
/// exist at session-open time, the `.failure/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.failure/` followed by a
/// write to `<cwd>/.failure/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
        ...
        .watch(&grok_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .failure directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    if let Err(e) = debouncer.watcher().unwatch(&grok_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .failure directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.failure/` hasn't
...
const HOME_VENDOR_DIRS: &[&str] = &[".failure", ".agents", ".claude", ".cursor"];
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_grok_home_and_vendor_dirs() {
        ...
        let home = home.path();
        let grok_home = home.join(".failure");
    ...
    /// A write to `<cwd>/.failure/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".failure");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.failure/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.failure/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".failure");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
