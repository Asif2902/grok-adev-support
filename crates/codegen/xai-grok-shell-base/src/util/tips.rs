...
//! persisted to `~/.adevgrok/tip_cursor.json`.
...
/// Load the cursor from `~/.adevgrok/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.adevgrok/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.adevgrok/tip_cursor.json`, so every session sees the next tip
