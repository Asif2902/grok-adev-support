...
- **`adevgrok dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.adevgrok/config.toml` under `[keybindings]` like every other shortcut.
...
```
 ADEVGrok · Dashboard — 4 agents · 2 awaiting
...
The state icon matches ADEVGrok's sibling views (
`tasks_pane`):
...
rebound via `~/.adevgrok/config.toml`.
...
Set `FAILURE_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `adevgrok --bg`") would list
