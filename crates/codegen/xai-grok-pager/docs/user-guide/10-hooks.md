...
Hooks let you run a script or send an HTTP request at key moments in a ADEVGrok session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that ADEVGrok calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.adevgrok/hooks
   ```

2. Create a hook file, e.g. `~/.adevgrok/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'ADEVGrok session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a ADEVGrok session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.adevgrok/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.adevgrok/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.adevgrok/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.adevgrok/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.adevgrok/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.
...
ADEVGrok accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). ADEVGrok skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
- **timeout**: Seconds before killing the hook (default: 5). All hook failures (timeouts, crashes, malformed output, missing required env vars) are fail-open: the adevgrok is recorded for the UI scrollback but the tool call is not blocked. Only an explicit `deny` decision returned by the hook blocks a tool call.

### Tool Name Aliases

In a `matcher`, ADEVGrok maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
| Other | Fail-open — the adevgrok is recorded but the tool call is not blocked. To block a call, emit a `deny` decision in stdout JSON (honored regardless of exit code). |
...
ADEVGrok sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
| `FAILURE_SESSION_ID`     | The unique identifier of the current ADEVGrok session. |
...
When a hook originates from a plugin, ADEVGrok additionally injects the following variables:
...
Press `r` in the Hooks tab to reload all hooks from disk. ADEVGrok re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.adevgrok/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.adevgrok/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug FAILURE_LOG_FILE=/tmp/adevgrok.log adevgrok`, then check `/tmp/adevgrok.log`.
