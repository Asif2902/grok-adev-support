...
Headless mode runs ADEVGrok non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
ADEVGrok processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
adevgrok -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
adevgrok -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
adevgrok -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
adevgrok -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
adevgrok -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
adevgrok -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
adevgrok -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
adevgrok -p "Build the project" --allow "Bash"
```
...
On adevgrok, ADEVGrok emits an error object (process exit non-zero). Prompt-level
...
ADEVGrok may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `adevgrok -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
adevgrok -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
adevgrok -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
adevgrok -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
adevgrok -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
adevgrok -p "What's the secret number?" --resume abc123
```
...
```bash
adevgrok -p "Continue where we left off" -c
```
...
```bash
adevgrok -p "Hello" --output-format json | jq -r '.sessionId'
```
...
adevgrok -p "Generate a README" > README.md

# Parse JSON output with jq
adevgrok -p "List files" --output-format json | jq -r '.text'
```
...
adevgrok -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
adevgrok --prompt-file ./prompt.txt
```
...
```bash
adevgrok -p "Review changes for bugs and security issues." \
...
```bash
adevgrok -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
for file in src/*.js; do
  adevgrok -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
ADEVGrok's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
class GrokChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["adevgrok", "-p", prompt, "-m", model, "--cwd", self.cwd,
...
```bash
#!/bin/bash
# Run a code review and exit with adevgrok if issues are found

RESULT=$(adevgrok -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
adevgrok -p "Format all files" --yolo

# Run tests and fix failures
adevgrok -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| `FAILURE_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
...
adevgrok -p "Run the test suite" --yolo
```
...
| `1`  | Error -- authentication adevgrok, network error, or runtime error |
...
- **`adevgrok login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`adevgrok login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `adevgrok -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug adevgrok -p "..." 2> debug.log`.

---

## Project Root Discovery

When ADEVGrok starts, it discovers the project root by walking upward from `--cwd`
...
ADEVGrok discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
ADEVGrok stores data in `~/.adevgrok` (override with `FAILURE_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.adevgrok` read-only:
...
```bash
export XAI_API_KEY="xai-..."
export FAILURE_DISABLE_AUTOUPDATER=1
adevgrok -p "..." --no-auto-update
```
...
| `FAILURE_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`FAILURE_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `FAILURE_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$FAILURE_HOME/bin/adevgrok`).
...
- Resume: `adevgrok -p "continue" --resume "<id>"` or `adevgrok -p "continue" --continue`
