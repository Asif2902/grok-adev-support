...
2. Create a simple hook file, e.g. `~/.adevgrok/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.adevgrok/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.adevgrok/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
- Any other (including timeout/crash/missing env var) — **fail-open**: the adevgrok is logged and shown in the hook scrollback, but the tool call is not blocked. To block a tool call, return JSON `{"decision":"deny","reason":"..."}` on stdout.
...
- `FAILURE_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `FAILURE_HOOK_NAME` — the full configured name of this hook
- `FAILURE_SESSION_ID` — the current session identifier
- `FAILURE_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `FAILURE_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `FAILURE_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `FAILURE_PLUGIN_ROOT` and `FAILURE_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`FAILURE_PLUGIN_ROOT` and `FAILURE_PLUGIN_DATA`. These keys override any user-declared
...
`${FAILURE_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.adevgrok/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.adevgrok/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.adevgrok/logs`).
...
Copy them to `~/.adevgrok/hooks/` and customize.
