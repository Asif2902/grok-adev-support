#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.adevgrok` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.adevgrok` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.adevgrok` reads).
    #[test]
