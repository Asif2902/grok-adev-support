...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `FAILURE_AGENT`,
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".failure").join("personas")),
...
/// True when `path` is a deletable local persona file (user or project `.failure/personas`).
...
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".failure").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".failure").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".failure").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.failure/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".failure").join("personas").join("gone.toml");
