...
/// `~/.adevgrok/config.toml`. Lenient — corrupted fields fall back to
...
/// Load the full persisted dashboard from `~/.adevgrok/config.toml`.
///
...
/// `~/.adevgrok/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
mod tests {
    ...
    /// per-key serial lock. The `FAILURE_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(FAILURE_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `FAILURE_AGENT_DASHBOARD` key reads it concurrently.
