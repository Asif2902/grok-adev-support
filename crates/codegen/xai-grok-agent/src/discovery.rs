...
/// Project-level agent directories to scan (`.failure/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".failure/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.failure/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.failure/agents` / `.claude/agents`) under each
...
///
/// Project-level `.failure/agents/` has highest priority, then falls back
...
/// Load project agent definitions from every `.failure/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_discover_finds_md_files() {
        ...
        let agents_dir = tmp.path().join(".failure").join("agents");
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".failure").join("agents");
        let agents_dir_2 = inner_dir.join(".failure").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".failure").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".failure")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".failure")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".failure").join("agents");
        let bundled_dir = home.join(".failure").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".failure")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".failure")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".failure").join("agents");
        let bundled_dir = home.join(".failure").join("bundled").join("agents");
