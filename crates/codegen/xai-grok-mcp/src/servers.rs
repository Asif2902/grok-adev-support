...
pub struct McpState {
    ...
    /// Persisted to `~/.adevgrok/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
/// Copy an MCP server's stderr to `~/.adevgrok/logs/mcp/<server>.stderr.log`
...
mod tests {
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("FAILURE_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("grok_com_FIGMA", "https://example.com/mcp"));
