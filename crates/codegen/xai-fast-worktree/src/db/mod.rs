...
#[cfg(test)]
static FAILURE_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`FAILURE_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `FAILURE_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `FAILURE_HOME` before `_lock` releases, so the env is correct
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = FAILURE_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("grok-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via FAILURE_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until FAILURE_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("FAILURE_HOME");
        unsafe { std::env::set_var("FAILURE_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
    ...
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("FAILURE_HOME", p),
                None => std::env::remove_var("FAILURE_HOME"),
            }
        }
    }
}
