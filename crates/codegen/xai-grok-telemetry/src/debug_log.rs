...
//!   firehose to `~/.adevgrok/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `FAILURE_DEBUG_LOG=1` → route per session into `<dir>` (`~/.adevgrok/debug`).
...
/// per session into `~/.adevgrok/debug`, an explicit path writes a single file.
///
...
/// `~/.adevgrok/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
