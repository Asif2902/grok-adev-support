...
//! file logger that writes to `~/.adevgrok/logs/memory.log`.
//!
//! ## When to use
//!
...
//! FAILURE_MEMORY_LOG=0 grok                # disable even when enabled
//! tail -f ~/.adevgrok/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "FAILURE_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.adevgrok/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `FAILURE_MEMORY_LOG=0` to disable, `FAILURE_MEMORY_LOG=/path` to redirect.
