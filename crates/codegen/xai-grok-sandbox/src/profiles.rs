...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Project config: <workspace>/.failure/sandbox.toml (additive only)
    let project_path = workspace.join(".failure").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    let project =
        load_config_file(&workspace.join(".failure").join("sandbox.toml")).unwrap_or_default();
