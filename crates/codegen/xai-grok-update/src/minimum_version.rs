...
//! `max(latest, minimum)`; otherwise the user is asked to run `adevgrok update`.
//!
//! Set `FAILURE_TEST_VERSION` to manually exercise either path without producing
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "This version of Grok ({current}) is no longer supported. \
         Run `adevgrok update` to install version {minimum} or later."
    )]
    ...
    #[error(
        "This version of Grok ({current}) is no longer supported, \
         ...
         Run `adevgrok update` to try again."
    )]
    UpgradeFailed {
        current: String,
        minimum: String,
    ...
    #[error(
        "Cannot install Grok {target}: the configured minimum is {minimum}. \
         Run `adevgrok update` to install the latest allowed version."
    )]
...
/// Used by `adevgrok update` to keep the install target at or above the pin.
...
///
/// `FAILURE_TEST_VERSION` lets devs override the running version to skip
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("FAILURE_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("FAILURE_TEST_VERSION", "0.1.50") };
        let decision =
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("FAILURE_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("FAILURE_TEST_VERSION") },
        }
    }
}
