...
//! (requirements > env `FAILURE_TOOLS_FIND_BFS` / `FAILURE_TOOLS_GREP_UGREP` (+
//! `FAILURE_FIND_BFS` / `FAILURE_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("FAILURE_TOOLS_BFS_VER"),
    "-",
    env!("FAILURE_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("FAILURE_TOOLS_UGREP_VER"),
    "-",
    env!("FAILURE_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "FAILURE_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "FAILURE_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}
...
mod tests {
    ...
    /// mode-0644 `FAILURE_TOOLS_*_PATH` / vendor copy) must fall through to the OS
