...
///
/// `skills` is the standard layout (`.failure/skills/`, `.claude/skills/`,
...
/// `cwd` (exclusive). At each directory, checks for `.failure/skills/`,
...
/// visits known config roots (`.failure`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.failure` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".failure", ".agents"];
...
";
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.failure/ → kept (user content).
        let grok_shell = tmp.path().join(".failure").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.failure/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .failure skill in an intermediate dir.
        ...
        .unwrap();
        let grok_skill = sub.join(".failure").join("skills").join("grok-dyn");
