//! LSP server configuration from `.failure/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".failure").join("lsp.json");
...
/// Load LSP server configs from `~/.adevgrok/lsp.json` and `<cwd>/.failure/lsp.json`.
