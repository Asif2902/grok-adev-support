...
//! after every tool call to check for SKILL.md files in `.failure/skills/`,
...
/// Directories that contain skill definitions (`.failure/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".failure", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.failure/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.failure/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.adevgrok/) are outside the git root so the walk breaks early.
