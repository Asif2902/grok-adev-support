...
/// `FAILURE_WORKSPACE_TERMINATION_GRACE_MS`. 45s fits under the K8s grace period.
const DEFAULT_TERMINATION_GRACE_MS: u64 = 45_000;
/// preStop-hook drain marker; override via `FAILURE_WORKSPACE_DRAINING_FILE`.
...
impl WorkspaceHandle {
    ...
    /// Construct a handle with an explicit `$FAILURE_WORKSPACE_HOME` and a
    ...
    /// `FAILURE_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `FAILURE_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
...
/// The SIGTERM drain budget from `FAILURE_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("FAILURE_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`FAILURE_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("FAILURE_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
/// sandbox; override via `FAILURE_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    ...
    let api_base_url = std::env::var("FAILURE_CLI_CHAT_PROXY_BASE_URL")
        .unwrap_or_else(|_| "https://cli-chat-proxy.grok.com/v1".to_string());
    let data_collection_disabled =
        std::env::var("FAILURE_WORKSPACE_DATA_COLLECTION_DISABLED").as_deref() != Ok("false");
    ...
    if let Ok(dir) = std::env::var("FAILURE_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("FAILURE_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("FAILURE_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
...
/// Resolve `$FAILURE_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
...
/// (`FAILURE_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
...
fn events_enabled() -> bool {
    std::env::var("FAILURE_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `FAILURE_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("FAILURE_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`FAILURE_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("FAILURE_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
/// only [`connect_local_workspace`] resolves `$FAILURE_WORKSPACE_HOME` — so the
...
/// Resolve `workspace_rewind_all_outcomes` from `FAILURE_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    xai_grok_config::env_bool("FAILURE_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
pub(crate) mod tests {
    ...
    /// `$FAILURE_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $FAILURE_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("FAILURE_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("FAILURE_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
