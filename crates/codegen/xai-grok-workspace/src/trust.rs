...
//!
//! Persists per-folder trust decisions to `~/.adevgrok/trusted_folders.toml`.
...
//! home that resolves to `None` (rather than a cwd-relative `./.failure`) when
//! neither `$FAILURE_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.failure/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.adevgrok/`.
...
impl TrustStore {
    ...
    /// `./.failure` — that fallback would let an untrusted cloned repo's `.failure`
...
/// ROOT (via the `~/.adevgrok/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.adevgrok/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.adevgrok/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.failure");
        assert_eq!(
        ...
        // cwd-relative `./.failure` that grok_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.failure/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
    ...
    /// Point `FAILURE_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `FAILURE_HOME` on
    ...
    fn register_grok_worktree(
        ...
        let env = LockedTestEnv::lock().set("FAILURE_HOME", &home);
