...
/// the metadata DB always live under the same `.failure` tree. That resolver
...
/// Returns `~/.adevgrok/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.adevgrok/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.adevgrok/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.adevgrok/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.adevgrok/worktrees/`.
...
///
/// When the source path is already inside `~/.adevgrok/worktrees/<repo>/`, the
...
/// Scan `~/.adevgrok/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    /// Point `FAILURE_HOME` at an isolated tempdir (`resolve_grok_home` re-reads
    ...
    /// and restores `FAILURE_HOME` on drop (before releasing the lock), so the
