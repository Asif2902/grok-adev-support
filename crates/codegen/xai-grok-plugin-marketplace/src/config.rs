//! Parse marketplace sources from `~/.adevgrok/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.adevgrok/` and `~/.claude/`.
...
/// instead of `~/.adevgrok`/`~/.claude`. Each root is checked for
